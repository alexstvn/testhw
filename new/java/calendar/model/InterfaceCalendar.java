...
public interface InterfaceCalendar {
  ...
  /**
   * Finds the series that contains an event with the given subject, start time, and end time.
   *
   * @param subject the event subject
   * @param start   the start date and time of the event
   ...
   * @return the series that contains the specified event, null if the event is not part of a series
   */
  InterfaceSeries findSeriesForSpecificEvent(String subject, LocalDateTime start,
                                             LocalDateTime end);
}
