...
public class MonthTablePanel extends JPanel implements InterfaceMonthTablePanel {
  ...
  private Map<LocalDate, List<InterfaceViewEvent>> eventsMap;
  private Map<LocalDate, JPanel> dateCellPanels;
  private int currentYear;
  private int currentMonth;
  private Consumer<LocalDate> selectDayConsumer;

  /**
   * Initializes a month view of the calendar.
   */
  ...
  @Override
  public void setEvents(Map<LocalDate, List<InterfaceViewEvent>> events) {
  ...
  private void addDateCells() {
    ...
    for (int i = 0; i < CALENDAR_CELLS; i++) {
      ...
      List<InterfaceViewEvent> dayEvents = eventsMap.getOrDefault(date, new ArrayList<>());
  ...
  private JPanel createDateCell(LocalDate date, boolean isToday,
                                boolean isCurrentMonth, List<InterfaceViewEvent> events) {
  ...
  public void setSelectDayConsumer(Consumer<LocalDate> consumer) {
    this.selectDayConsumer = consumer;
  }

  private void onDaySelected(LocalDate date) {
    if (selectDayConsumer != null) {
      selectDayConsumer.accept(date);
    }
  }
  ...
  private void addEventsList(JPanel cellPanel, List<InterfaceViewEvent> events) {
  ...
  private JLabel createEventLabel(InterfaceViewEvent event) {
